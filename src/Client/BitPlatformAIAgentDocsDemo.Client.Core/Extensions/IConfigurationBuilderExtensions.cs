...
public static partial class IConfigurationBuilderExtensions
{
    ...
    public static IConfigurationBuilder AddClientConfigurations(this IConfigurationBuilder builder, string clientEntryAssemblyName)
    {
        ...
        var sharedAssembly = Assembly.Load("BitPlatformAIAgentDocsDemo.Shared");

        configBuilder.AddJsonStream(sharedAssembly.GetManifestResourceStream("BitPlatformAIAgentDocsDemo.Shared.appsettings.json")!);

        var envSharedAppSettings = sharedAssembly.GetManifestResourceStream($"BitPlatformAIAgentDocsDemo.Shared.appsettings.{AppEnvironment.Current}.json");
        if (envSharedAppSettings != null)
        {
            configBuilder.AddJsonStream(envSharedAppSettings);
        }

        var clientCoreAssembly = AppDomain.CurrentDomain.GetAssemblies().Single(asm => asm.GetName()?.Name is "BitPlatformAIAgentDocsDemo.Client.Core");

        configBuilder.AddJsonStream(clientCoreAssembly.GetManifestResourceStream("BitPlatformAIAgentDocsDemo.Client.Core.appsettings.json")!);

        var envClientCoreAppSettings = clientCoreAssembly.GetManifestResourceStream($"BitPlatformAIAgentDocsDemo.Client.Core.appsettings.{AppEnvironment.Current}.json");
